// Import API service
import * as apiService from './api_service.js';
...
function loadQuotes() {
    // Use the imported apiService to load backup quotes
    apiService.fetchQuotes()
        .then(data => {
            quotesData = data;
            // Don't show a random quote here, we'll fetch from external API instead
            showRandomQuote(); // This will now use the external API
        })
        .catch(error => {
            console.error('Error loading backup quotes:', error);
            // Still try to show a random quote from the external API
            showRandomQuote();
        });
}
...
async function showRandomQuote() {
    try {
        ...
        console.log('Fetching a new quote from external API...');
        currentQuote = await apiService.fetchRandomQuote();
        console.log('Received quote:', currentQuote);

        // Ensure the quote has an ID (needed for favorite functionality)
        if (!currentQuote.id) {
            currentQuote.id = `quote_${Date.now()}`;
        }
        ...
        // Ensure the quote has an ID (needed for favorite functionality)
        if (!currentQuote.id) {
            currentQuote.id = `quote_${Date.now()}_${randomIndex}`;
        }
...
async function toggleFavoriteQuote() {
    ...
    try {
        ...
        const isFavorite = await apiService.checkFavoriteQuote(currentQuote.id);

        if (isFavorite) {
            // Remove from favorites
            await apiService.removeFavoriteQuote(currentQuote.id);
            showNotification('Removed from favorites');
        } else {
            // Add to favorites
            await apiService.addFavoriteQuote(currentQuote);
...
async function renderFavoriteQuotes() {
    ...
    try {
        ...
        const favoriteQuotes = await apiService.fetchFavoriteQuotes();
        ...
        favoriteQuotes.forEach(quote => {
            ...
            quoteCard.innerHTML = `
                ...
                <div class="favorite-quote-actions">
                    <button class="remove-favorite" data-id="${quote.id}"><i class="fas fa-trash"></i></button>
                    <button class="share-favorite" data-id="${quote.id}"><i class="fas fa-share-alt"></i></button>
                </div>
            `;
        ...
        favoritesContainer.innerHTML = '<p class="error">Failed to load favorite quotes. Please try again later.</p>';
    }
}

async function removeFavorite(quote) {
    try {
        // Remove from favorites
        await apiService.removeFavoriteQuote(quote.id);
...
async function loadWeatherMoodData() {
    try {
        // Fetch weather moods
        const data = await apiService.fetchWeatherMoods();
        weatherData = data;
...
async function loadMoodData() {
    try {
        // Fetch mood entries
        const data = await apiService.fetchMoodEntries();
        moodData = data;
        // Update visualization after loading data
        updateMoodHeatmap();
    } catch (error) {
...
async function saveMoodData() {
    ...
    if (latestMood) {
        try {
            ...
            console.error('Error saving mood data to API:', error);
        }
    }
}
...
async function saveGratitudeEntry() {
    ...
    try {
        // Save entry to API
        const savedEntry = await apiService.saveGratitudeEntry({
            content: entryText,
            tags: selectedTags
        });

        // Add to local data for immediate UI update
        gratitudeData.unshift(savedEntry);
        ...
        // Show success notification
        showGratitudeNotification('Gratitude entry saved locally!', 'success');
...
async function loadGratitudeData() {
    try {
        ...
        const data = await apiService.fetchGratitudeEntries();
        gratitudeData = data;
...
async function saveGratitudeData() {
    ...
    // Then save to the API if it's a new entry
    const latestEntry = gratitudeData[0]; // Entries are added to the beginning of the array
    if (latestEntry) {
        try {
            ...
            console.error('Error saving gratitude entry to API:', error);
        }
    }
}
...
function renderGratitudeEntries(filter = 'all') {
    ...
    filteredEntries.forEach(entry => {
        // Format date - handle both string dates and array dates from MongoDB
        let entryDate;
...
async function deleteGratitudeEntry(id) {
    if (confirm('Are you sure you want to delete this entry?')) {
        try {
            ...
            await apiService.deleteGratitudeEntry(id);
            ...
            // Show notification
            showGratitudeNotification('Entry deleted locally', 'success');
        }
    }
}
...
async function updateGratitudeEntry(id) {
    const entry = gratitudeData.find(entry => entry.id === id);
    if (!entry) return;
    ...
    try {
        // Update entry in API
        const updatedEntryData = await apiService.updateGratitudeEntry(id, updatedEntry);

        // Update entry in local data
        const index = gratitudeData.findIndex(e => e.id === id);
        if (index !== -1) {
            gratitudeData[index] = updatedEntryData;
        }
        ...
        // Show notification
        showGratitudeNotification('Entry updated locally', 'success');
    }
}
...
async function updateGratitudeStats() {
    try {
        // Fetch stats from API
        const stats = await apiService.fetchGratitudeStats();

        // Update UI with stats from API
        document.getElementById('total-entries').textContent = stats.totalEntries;
        document.getElementById('current-streak').textContent = stats.currentStreak;
        document.getElementById('monthly-entries').textContent = stats.monthlyEntries;
    } catch (error) {
        console.error('Error fetching gratitude stats:', error);

        // Fallback to calculating stats locally
